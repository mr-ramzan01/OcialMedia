...
export const CreateNewPassword = () => {
  ...
  const [passwordError1, setPasswordError1] = useState({error: false, text: ''});
  const [passwordError2, setPasswordError2] = useState({error: false, text: ''});
  ...
  const passwordValidation = (name, value) => {
    if(name === 'password1' && value.length > 0) {
      if(validator.isStrongPassword(value, {
        ...
        setPasswordError1({...passwordError1, error: true, text: 'Password must includes at least one lowercase, uppercase, numbers and symbols.'})
        return false;
      }
    }
    if(name === 'password2' && value.length > 0) {
      if(validator.isStrongPassword(value, {
        ...
        setPasswordError2({...passwordError2, error: true, text: 'Password must includes at least one lowercase, uppercase, numbers and symbols.'})
        return false;
      }
    }
    return true;
  ...
  const handleSubmit = (event) => {
    ...
    if(data.password1 !== data.password2) {
      setPasswordError1({...passwordError1, error: true, text: "Password didn't match"})
      setPasswordError2({...passwordError2, error: true, text: "Password didn't match"})
    }
    else {
      changePassword();
    }
          ...
          <TextField
              ...
              InputProps={{
                endAdornment: (
                  <InputAdornment position="end">
                    <IconButton edge="end" color="primary" onClick={() => setPasswordVisibility1(!passwordVisibility1)}>
                      {passwordVisibility1?<MdVisibility />: <MdVisibilityOff />}
                    </IconButton>
                  </InputAdornment>
                ),
                style: {
            ...
            <TextField
              ...
              InputProps={{
                endAdornment: (
                  <InputAdornment position="end">
                    <IconButton edge="end" color="primary" onClick={() => setPasswordVisibility2(!passwordVisibility2)}>
                      {passwordVisibility2?<MdVisibility />: <MdVisibilityOff />}
                    </IconButton>
                  </InputAdornment>
                ),
                style: {
